...
function ErrorState({ error, refetch }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 13, color: 'var(--text3)', maxWidth: 440, margin: '0 auto 20px' }}>
        {error}
      </div>
      <div style={{ fontSize: 12, color: 'var(--text3)', background: 'var(--bg3)',
        padding: '12px 18px', borderRadius: 8, display: 'inline-block', marginBottom: 16 }}>
        Make sure your Google Sheet is published via <strong>File → Share → Publish to web</strong>
        and the published key in <code style={{ color: 'var(--accent)' }}>useSheets.js</code> is correct.
      </div>
      <br />
      <button
        onClick={refetch}
...
export default function App() {
  ...
  const { log,   loading: logLoading,   error: logError            } = useReadingLog()
